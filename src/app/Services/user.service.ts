...
export class UserService {
  ...
  private apiUrl = 'http://localhost:8089/DanceScape';
  ...
  authguard(): boolean {
    ...
    if (authToken) {
      ...
      if (token && token.exp) {
        ...
        return currentTime < token.exp; // Check if the token is still valid
      } else {
        return false; // Token is invalid or missing expiration
      }
    }
